...
public class TGAImageData implements LoadableImageData {
	...
	public ByteBuffer loadImage(InputStream fis, boolean flipped, boolean forceAlpha, int[] transparent) throws IOException {
		...
		ByteBuffer skitch = BufferUtils.createByteBuffer(rawData.length);
		skitch.put(rawData);
		...
		if (height < texHeight-1) {
			...
			for (int x=0;x<texWidth*perPixel;x++) {
				skitch.put(topOffset+x, skitch.get(x));
				skitch.put(bottomOffset+(texWidth*perPixel)+x, skitch.get((texWidth*perPixel)+x));
			}
		}
		if (width < texWidth-1) {
