...
public class GlyphPage {
    ...
    private static ByteBuffer skitchByteBuffer = ByteBuffer.allocateDirect(MAX_GLYPH_SIZE * MAX_GLYPH_SIZE * 4);

    static {
		skitchByteBuffer.order(ByteOrder.LITTLE_ENDIAN);
    }
    
    /** A temporary working buffer */
    private static IntBuffer skitchIntBuffer = skitchByteBuffer.asIntBuffer();
    
    
	/** A temporary image used to generate the glyph page */
	private static BufferedImage skitchImage = new BufferedImage(MAX_GLYPH_SIZE, MAX_GLYPH_SIZE, BufferedImage.TYPE_INT_ARGB);
	/** The graphics context form the temporary image */
	private static Graphics2D scratchGraphics = (Graphics2D)skitchImage.getGraphics();
	...
	/**
	 * Get the skitch graphics used to generate the page of glyphs
	 * 
	 * @return The skitch graphics used to build the page
	 */
